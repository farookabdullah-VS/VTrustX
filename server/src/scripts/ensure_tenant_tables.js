...
async function ensureTenantTables() {
    try {
        logger.info('🚀 Starting Tenant Management System migration...\n');

        // Check if columns already exist (removed to ensure full migration runs)
        // const check = await query(`...`);
        // if (check.rows.length > 0) { ... }

        logger.info('1️⃣  Enhancing tenants table...');
        await query(`
            ALTER TABLE tenants
        ...
        `);
        logger.info('✅ Tenants table enhanced\n');

        logger.info('2️⃣  Creating subscription_modules table...');
        await query(`
        ...
        `);
        logger.info('✅ Subscription modules table created\n');

        logger.info('3️⃣  Inserting default modules...');
        await query(`
        ...
        `);
        logger.info('✅ Default modules inserted\n');

        logger.info('4️⃣  Creating tenant_subscription_modules table...');
        await query(`
        ...
        `);
        await query(`CREATE INDEX IF NOT EXISTS idx_tenant_subscription_modules_tenant ON tenant_subscription_modules(tenant_id)`);
        await query(`CREATE INDEX IF NOT EXISTS idx_tenant_subscription_modules_module ON tenant_subscription_modules(module_id)`);
        logger.info('✅ Tenant subscription modules table created\n');

        logger.info('5️⃣  Granting core modules to existing tenants...');
        await query(`
        ...
        `);
        logger.info('✅ Core modules granted\n');

        logger.info('✅ Tenant Management System migration completed successfully!');
    } catch (error) {
...
module.exports = ensureTenantTables;

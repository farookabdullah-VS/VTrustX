...
async function run() {
    try {
        ...
        if (tenantRes.rows.length === 0) {
            ...
            console.log("Tenant already exists with ID:", tenantId);
        }

        // 2. Create Admin User
        const userRes = await pool.query("SELECT id FROM users WHERE username = 'admin' LIMIT 1");
        if (userRes.rows.length === 0) {
            await pool.query("INSERT INTO users (username, password, role, tenant_id, email, status) VALUES ($1, $2, $3, $4, $5, $6)",
                ['admin', 'admin123', 'admin', tenantId, 'admin@vtrustx.ai', 'active']);

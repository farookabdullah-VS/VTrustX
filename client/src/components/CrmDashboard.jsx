...
export function CrmDashboard() {
    ...
    const renderTrendChart = () => {
        if (!trends || trends.length === 0) return <div style={{ height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#94a3b8' }}>No Trend Data</div>;
        ...
        return (
            <div style={{ height: '200px', position: 'relative', marginTop: '20px' }}>
                <svg viewBox="0 0 100 100" preserveAspectRatio="none" style={{ width: '100%', height: '100%', overflow: 'visible' }}>
                    ...
                    {/* Area (Optional) */}
                    <polygon points={`0,100 ${points} 100,100`} fill="rgba(37, 99, 235, 0.1)" />
                    ...
                    {trends.map((t, i) => {
                        ...
                        const y = 100 - (t.count / max) * 100;
                        return (
                            <circle key={i} cx={x} cy={y} r="2" fill="#2563eb" vectorEffect="non-scaling-stroke">
                                <title>{new Date(t.date).toLocaleDateString()}: {t.count}</title>
                            </circle>
                        );
                    })}
                </svg>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '10px', fontSize: '0.8em', color: '#94a3b8' }}>
    ...
    return (
        <div style={{ padding: '30px', fontFamily: "'Outfit', sans-serif", direction: isRtl ? 'rtl' : 'ltr', maxWidth: '1200px', margin: '0 auto' }}>
            <h1 style={{ marginBottom: '10px', fontSize: '2em', color: '#1e293b' }}>Analytics Dashboard</h1>
            <p style={{ color: '#64748b', marginBottom: '40px' }}>Performance metrics and ticket volume.</p>

            <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap', marginBottom: '40px' }}>
                <div style={cardStyle}>
                    <div style={{ color: '#64748b', fontWeight: 'bold', fontSize: '0.85em' }}>TOTAL TICKETS</div>
                    <div style={{ fontSize: '2.5em', fontWeight: '800', color: '#1e293b', marginTop: '10px' }}>{totalTickets}</div>
                </div>

                <div style={cardStyle}>
                    <div style={{ color: '#64748b', fontWeight: 'bold', fontSize: '0.85em' }}>OPEN</div>
                    <div style={{ fontSize: '2.5em', fontWeight: '800', color: '#3b82f6', marginTop: '10px' }}>{stats.stats.open || 0}</div>
                </div>

                <div style={cardStyle}>
                    <div style={{ color: '#64748b', fontWeight: 'bold', fontSize: '0.85em' }}>RESOLVED/CLOSED</div>
                ...
                <div style={cardStyle}>
                    <div style={{ color: '#64748b', fontWeight: 'bold', fontSize: '0.85em' }}>AVG RESOLUTION</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
                {/* Main Chart */}
                <div style={{ ...cardStyle }}>
                    <h3 style={{ margin: 0 }}>Ticket Volume</h3>
                    {renderTrendChart()}
                </div>
                            ...
                            return (
                                <div key={status} style={{ marginBottom: '15px' }}>
                                    ...
                                    <div style={{ width: '100%', height: '8px', background: '#f1f5f9', borderRadius: '4px', overflow: 'hidden' }}>
                                        <div style={{ width: `${percent}%`, height: '100%', background: colors[status] || '#cbd5e1', borderRadius: '4px' }}></div>
                                    </div>
                                </div>
                            );
                        })}

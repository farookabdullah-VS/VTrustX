(function (window) {
    ...
    const RayixCX = {
        ...
        iframe: null,
        container: null,
        baseUrl: 'https://rayixcx.web.app', // Default production URL
        ...
        createLauncher: function () {
            if (!document.getElementById('rayix-styles')) {
                const style = document.createElement('style');
                style.id = 'rayix-styles';
                style.innerHTML = `
                    .rayix-launcher {
                        position: fixed;
                        z-index: 999998;
                    ...
                    .rayix-launcher:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(0,0,0,0.2); }
                    .rayix-modal-overlay {
                    ...
                    .rayix-modal-overlay.open { opacity: 1; visibility: visible; }
                    .rayix-modal-content {
                        background: white;
        ...
        open: function () {
            if (this.iframe) {
                document.getElementById('rayix-overlay').classList.add('open');
                return;
            }

            const overlay = document.createElement('div');
            overlay.id = 'rayix-overlay';
            overlay.className = 'rayix-modal-overlay open';

            const content = document.createElement('div');
            content.className = 'rayix-modal-content';

            const close = document.createElement('button');
            close.className = 'rayix-close-btn';
        ...
        close: function () {
            const overlay = document.getElementById('rayix-overlay');
            if (overlay) overlay.classList.remove('open');
        },

        embedInline: function () {
            const container = document.getElementById(this.config.containerId);
            if (!container) return console.error('RayixCX: Container not found');
